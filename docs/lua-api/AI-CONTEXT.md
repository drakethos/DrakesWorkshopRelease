...
**Boot:** `ApplySystem` - Apply a systems pack by id (advanced; prefer Systems tab). | `include` - Load a Lua lib/template/world script by name (deduped). | `Include` - Load a Lua lib/template/world script by name (deduped). | `Log` - Print a debug line to the game log.
...
**Other:** `ConsumeMatchIntroSkip` | `GetNetPlayerEntities` | `HideMatchIntro` | `IsNetHost` | `SetEntityOpacity` | `SetGameCursor` | `SetMatchIntroText` | `ShowMatchIntro`
